...
An interactive, microcontroller-based audio-visual project running CircuitPython / MicroPython. The device plays distinct audio feedback (`red.mp3`, `yellow.mp3`, `green.mp3`) corresponding to state changes, inputs, or color modes.
...
3. **Deploy Code & Assets**:
   * Copy `code/code.py` to the root of your `CIRCUITPY` drive.
   * Copy `green.mp3`, `red.mp3`, and `yellow.mp3` to the root directory alongside `code.py`.
4. **Run**: Reset or safely eject and power cycle the board. `code.py` executes automatically on boot.
